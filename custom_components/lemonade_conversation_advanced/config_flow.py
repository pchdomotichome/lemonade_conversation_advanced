...
STEP_MCP_DATA_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_LEMONADE_PORT, default=DEFAULT_MCP_PORT): vol.Coerce(int),
...
class LemonadeConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_advanced(
        ...
        if user_input is not None:
            ...
            # Validate MCP port
            mcp_port = user_input.get(CONF_LEMONADE_PORT, DEFAULT_MCP_PORT)
            if not 1024 <= mcp_port <= 65535:
                errors[CONF_LEMONADE_PORT] = "invalid_port"
            ...
            if not errors:
                ...
                if is_first_profile:
                    ...
                    if existing_entry:
                        shared_settings = {
                            CONF_LEMONADE_PORT: existing_entry.data.get(
...
class LemonadeOptionsFlow(config_entries.OptionsFlow):
    """Handle options flow for Lemonade Conversation Advanced integration."""
    ...
    async def async_step_mcp_server(self, user_input=None):
        ...
        mcp_schema = vol.Schema(
            {
                vol.Required(
                    ...
                    default=sys_options.get(
                        CONF_LEMONADE_PORT, sys_data.get(CONF_LEMONADE_PORT, DEFAULT_MCP_PORT)
                    ),
                ): vol.Coerce(int),
                vol.Required(
                    CONF_SEARCH_PROVIDER,
        ...
        return self.async_show_form(
            ...
            errors=errors,
            description_placeholders={
                "warning": "⚠️ These settings are shared across ALL Lemonade Conversation Advanced profiles. Restart Home Assistant after making changes."
            },
        )
