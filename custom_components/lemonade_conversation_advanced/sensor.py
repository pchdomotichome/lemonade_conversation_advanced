...
async def async_setup_entry(
    ...
    backend = entry.runtime_data.backend
    coordinator = LemonadeDataCoordinator(hass, backend)
    await coordinator.async_config_entry_first_refresh()
    async_add_entities([
        LemonadeSimpleSensor(
            coordinator,
            entry,
            SensorEntityDescription(
        ...
        LemonadeSimpleSensor(
            ...
            SensorEntityDescription(
                ...
                native_unit_of_measurement="tokens/s",
                state_class=SensorStateClass.MEASUREMENT,
...
class LemonadeDataCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, backend: Any) -> None:
        """Initialize coordinator."""
        super().__init__(
            hass,
            _LOGGER,
            name="Lemonade Server Data",
...
class LemonadeSimpleSensor(
    CoordinatorEntity[LemonadeDataCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: LemonadeDataCoordinator,
        entry: Any,
    ...
    def native_value(self):
        ...
        if self._key == "npu_usage":
            loaded = (data.system_info or {}).get("loaded_models", [])
            if any(
                model.get("device", "").lower() in ["npu", "ryzenai", "xdnn"]
                for model in loaded
            ):
                return "Active"
            return "Inactive"
